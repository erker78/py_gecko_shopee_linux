...
def getValueGogo(raw_data):
    try:
        return raw_data.find("a").getText().strip()
    except:
        return raw_data.find("div").getText().strip()

def getStringGogoWeight(str_gogo,regex):
    matches = re.finditer(regex, str_gogo, re.MULTILINE)
...
def getSpecification(str_gogo):
    ...
    #只取前30個字
    # str_gogo_item = str_gogo[0:30]
    str_gogo_item = str(str_gogo)

    for sample in sample_ary:
        sample_data = re.search(sample, str_gogo_item)
        if sample_data:
            return sample
    return ''
...
print("連線網址: " + str(r.url_main)) #查看連線的網址
...
while k <= page:
    ...
    for c, p, l in zip(contents, prices, links):
        ...
        for bu in buy_data:
            ...
            if title == '品牌':
                _Brand = [getValueGogo(bu)]

            elif title == '產地':
                _Origin = [getValueGogo(bu)]

            elif title == '商品數量':
                _Products_num = [getValueGogo(bu)]

            elif title == '出貨地':
                _Shipping_place = [getValueGogo(bu)]

            elif title == '運送限制':
                _Shipping_restrictions = [getValueGogo(bu)]

            elif title == '保存期限':
                _Shelf_life = [getValueGogo(bu)]

            elif title == '種類':
                _Species = [getValueGogo(bu)]

            elif title == '豬肉產地':
                _Pork_origin = [getValueGogo(bu)]

            elif title == '規格':
                _Specification = [getValueGogo(bu)]

            elif title == '內容物/成分':
                _Contents = [getValueGogo(bu)]

            elif title == '有效日期/剩餘保存期限':
                _Effective_date = [getValueGogo(bu)]

            elif title == '食品業者登錄字號':
                _Food_business = [getValueGogo(bu)]

            elif title == '國內負責廠商電話':
                _Telephone = [getValueGogo(bu)]

            elif title == '產品責任險':
                _Product_Insurance = [getValueGogo(bu)]
        ...
        if _Product_name == ['']:
            for _Product_name_num in range(10):
                try:
                    _Product_name = [getSpecification(c.contents[0])]
                    break
                except:
                    _Product_name = ['']
        ...
        if _Specification == ['']:
            try:
                regex = r"^(?:內容物|產品規格)\W{1,3}(.*)"
                _Specification = [getStringGogoWeight(str(buy_detail.getText()), regex)]
            except:
                _Specification = ['']
        ...
        try:
            ...
            _Weight = [getStringGogoWeight(str(buy_detail.getText()), regex)]
        except:
            _Weight = ['']
        ...
        if _Weight == ['']:
            try:
                regex = r"(\d+g)|(\d+\.\d+kg)"
                _Weight = [getStringGogoWeight(c.contents[0], regex)]
            except:
                _Weight = ['']
        ...
        if _Shelf_life == ['']:
            try:
                regex = r"^有效期限(.*)"
                _Shelf_life = [getStringGogoWeight(str(buy_detail.getText()), regex)]
            except:
                _Shelf_life = ['']
        ...
        if _Origin == ['']:
            try:
                regex = r"^(?:\*產地|產地|★ 產地|◎原產地\(國家\)|原產地\(國\))\W{1,3}(.*)"
                _Origin = [getStringGogoWeight(buy_detail.getText(), regex)]
            except:
                _Origin = ['']
